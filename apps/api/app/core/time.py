"""Timeframe helpers compartidos entre módulos (agente, market, alerts, etc.).

Anteriormente vivían en `app.agent.tools._time` y eran importados desde la
capa de datos (`market/ohlcv/repo.py`, `market/ohlcv/ingestion_live.py`) — una
violación de capas: la capa de datos NO debe depender de la capa de
consumidor. Movidas aquí en el audit fix 2026-05 para invertir la dirección.

`agent/tools/_time.py` queda como re-export para compatibilidad temporal.
"""
...
def staleness_warning(
    *, last_closed: datetime, timeframe: str, now: datetime | None = None
) -> str | None:
