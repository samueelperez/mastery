...
async def _run() -> None:
    ...
    async with session_scope() as session:
        ...
        stale: list[tuple[str, str, int]] = []  # (id, new_summary, new_version)
        for r in rows:
            ...
            if new_hash != r.summary_hash:
                stale.append((r.id, new_summary, r.embedding_version + 1))
        ...
        for i in range(0, len(stale), BATCH):
            ...
            new_summaries = [s for (_, s, _) in chunk]
            embeddings = await embed_batch(new_summaries, input_type=INPUT_TYPE_DOCUMENT)
            for (trade_id, summary, ver), emb in zip(chunk, embeddings, strict=True):
                await update_summary_and_embedding(
                    session,
                    trade_id=trade_id,
                ...
                refreshed += 1
            log.info("embed_backfill.batch", offset=i, refreshed=refreshed)
