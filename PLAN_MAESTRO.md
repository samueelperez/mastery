...
|---------|--------|-------|
| Scout (binary decisiones rápidas) | `claude-haiku-4-5-20251001` | Latencia <1s, coste 1/10 de Sonnet |
| Chat operador + Supervisor (reviewer+post_mortem fusionados) | `anthropic/claude-sonnet-4-6` | Razonamiento multi-paso, citation rigor |
| Audit on-demand manual | `anthropic/claude-opus-4-7` | Análisis profundo de decisiones controversiales |

Tools en el agente principal: **8** (reducidas de 22 originales). Lista alfabética:
1. `get_basis`
2. `get_bias`
3. `get_indicators`
4. `get_journal_query`
5. `get_liquidation_heatmap` ← Cerebro 1
6. `get_market_dominance`
7. `list_alerts`
8. `list_open_setups`

Razón para reducir de 22 a 8: cada tool extra añade ~150 tokens al system prompt (caché) + degrada la atención del LLM al elegir entre opciones. 8 tools es el punto óptimo según experimentos de eval interna.

Reviewer y post_mortem (originalmente 2 agentes separados) están **fusionados en un único Supervisor** que actúa como segundo par de ojos pre-aprobación y como post-mortem post-cierre. Mismo prompt, mismo state, dos modos de invocación.
...
- Implementar Cerebro 1 según los 9 specs en `docs/specs/liquidation/`.
...
| 22 tools en main agent | Degrada atención del LLM. Reducido a 8. |
| reviewer/ y post_mortem/ como agentes separados | Mismo system prompt, mismo state. Fusionados en Supervisor único. |
